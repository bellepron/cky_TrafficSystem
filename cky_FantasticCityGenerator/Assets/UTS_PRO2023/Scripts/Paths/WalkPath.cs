...
public class WalkPath : MonoBehaviour
{
    ...
    [Tooltip("Objects of motion")] public GameObject[] walkingPrefabs;
    [Tooltip("Number of paths")] public int numberOfWays = 1;
    [Tooltip("Space between paths")] public float lineSpacing = 1;
    [Tooltip("Density of movement of objects")][Range(0.01f, 0.50f)] public float Density = 0.02f;
    [Tooltip("Distance between objects")][Range(1f, 10f)] public float _minimalObjectLength = 1f;
    [Tooltip("Make the path closed in the ring")] public bool loopPath;
    [SerializeField] protected EnumDir direction;
    ...
    [Tooltip("Radius of the sphere-scraper"), Range(0.1f, 25)]
    public float eraseRadius = 2f;

    [Tooltip("The minimum distance from the cursor to the line at which you can add a new point to the path")][Range(0.5f, 10)] public float addPointDistance = 2f;

    [Tooltip("Adjust the spawn of cars to the nearest surface. This option will be useful if there are bridges in the scene")] public float highToSpawn = 1.0f;
    ...
    public virtual void DrawCurved(bool withDraw, EnumDir direct = EnumDir.Forward)
    {
        ...
        {
            if (direct == EnumDir.Forward)
            {
                _forward[w] = true;
            }
            else if (direct == EnumDir.Backward)
            {
                _forward[w] = false;
            }
            else if (direct == EnumDir.HugLeft)
            {
                if ((w + 2) % 2 == 0)
                    _forward[w] = true;
                else
            ...
            else if (direct == EnumDir.HugRight)
            {
                if ((w + 2) % 2 == 0)
                    _forward[w] = false;
                else
            ...
            else if (direct == EnumDir.WeaveLeft)
            {
                if (w == 1 || w == 2 || (w - 1) % 4 == 0 || (w - 2) % 4 == 0)
                    _forward[w] = false;
                else _forward[w] = true;
            }
            else if (direct == EnumDir.WeaveRight)
            {
                if (w == 1 || w == 2 || (w - 1) % 4 == 0 || (w - 2) % 4 == 0)
                    _forward[w] = true;
