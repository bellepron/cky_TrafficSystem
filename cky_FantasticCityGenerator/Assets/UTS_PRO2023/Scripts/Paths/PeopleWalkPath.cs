...
using cky.UTS.People.Passersby;
using cky.UTS.Car;

public class PeopleWalkPath : WalkPath
{
    [Range(0.0f, 5.0f)][Tooltip("Offset from the line along the X axis")] public float randXPos = 0.1f;
    [Range(0.0f, 5.0f)][Tooltip("Offset from the line along the Z axis")] public float randZPos = 0.1f;

    [HideInInspector][SerializeField][Tooltip("Ignore pedestrian colliders?")] private bool _ignorePeople = true;
    [HideInInspector][SerializeField][Tooltip("Ignore the colliders of other pedestrians")] private bool _ignoreCar = true;
    [HideInInspector][SerializeField][Tooltip("Ignore the colliders of other pedestrians")] private bool _ignoreBicycle = true;
    private float nextPointThreshold = 1;

    #region Passerby Properties
    [SerializeField] PasserbySettings passerbySettings;
    #endregion
    ...
    public override void CreateSpawnPoints() // Editör
    {
    ...
    public override void SpawnOnePeople(int w, bool forward)
    {
        ...
        var passerby = people.GetComponent<PasserbyStateMachine>();

        passerby.Init(passerbySettings, movePath, transform.name);
        ...
        movePath.walkPointThreshold = nextPointThreshold;
    }

    public override void SpawnPeople() // Editör
    {
        if (passerbySettings == null)
        {
            passerbySettings = Resources.Load<PasserbySettings>("PasserbySettings/DefaultPasserbySettings");
        }
        ...
        for (int wayIndex = 0; wayIndex < numberOfWays; wayIndex++)
        {
            ...
            for (int peopleIndex = 0; peopleIndex < peopleCount; peopleIndex++)
            {
                ...
                {
                    Collider collider = rrr[i].collider;
                    if (collider.GetComponent<SemaphoreSystem>() != null ||
                        collider.GetComponent<SemaphoreMovementSide>() != null ||
                        collider.GetComponent<TSemaphoreSystem>() != null)
                    {
                        isSemaphore = true;
                    }
                }
