...
//using UnityEngine;
//using static FCG.Pedestrians.PedestrianTrafficSystemBackUp;

//namespace FCG.Pedestrians
//{
//    public class PedestrianTrafficSystemBackUp : MonoBehaviour
//    {
...
//                        if (!_isFirstTime && InTheFieldOfVision(posPlayer, posData))
//                            continue;
//                    }
...
//                        go = true; //|| !player;
//                    else
//                        go = !Physics.Linecast(wpDataSpawn_WayScript.Node(wpDataSpawn_Side, wpDataSpawn_Node + 1) + Vector3.up * 1f,
//                                               wpDataSpawn_WayScript.Node(wpDataSpawn_Side, wpDataSpawn_Node) + Vector3.up * 1f, out obsRay2);

//                    if (go)
//                    {
//                        human = (GameObject)Instantiate(pedestrianPrefabs[Mathf.Clamp(Random.Range(0, pedestrianPrefabs.Length), 0, pedestrianPrefabs.Length - 1)], wpDataSpawn.position, wpDataSpawn.rotation);
//                        human.transform.SetParent(PedestrianContainer.transform);

//                        var tPedestrian = human.GetComponent<PedestrianStateMachine>();
//                        tPedestrian.sideAtual = sa;
//                        tPedestrian.atualWay = aw;
//                        tPedestrian.atualWayScript = wpDataSpawn_WayScript;
//                        tPedestrian.currentNode = wpDataSpawn_Node + 1;
