...
namespace FCG
{
    public class TrafficCar : MonoBehaviour
    {
        ...
        [HideInInspector] public bool nodeSteerCarefully = false;     //true if I'm turning to the side that has opposite traffic (Right hand and turning left) Or (Left hand and turning right)
        [HideInInspector] public bool nodeSteerCarefully2 = false;    // For double line oneway
        ...
        float FixedRaycasts()
        {
            ...
            if (rStop > 0 && speed < 2)
            {
                ...
                else if (hit.transform.CompareTag("Stop")) // Trafik ışığında durması için
                {
                    if (hit.transform.TryGetComponent<Crosswalk>(out var crosswalk))
                    {
                        if (!crosswalk.CarPass)
